...
@group(0) @binding(3) var<storage, read> matrix: array<vec4<f32>>;          // (B, R, C)
...
fn matmul(@builtin(global_invocation_id) invocation_id: vec3<u32>) {
    ...
    for (var i = index; i < stride; i += BLOCK_SIZE) {
        ...
        // read 4 rows from the matrix, each with 4 floats, forming a 4x4 sub-block
        var m: mat4x4<f32>;

        m[0] = matrix[ci]; ci += stride;
        m[1] = matrix[ci]; ci += stride;
        m[2] = matrix[ci]; ci += stride;
        m[3] = matrix[ci];
